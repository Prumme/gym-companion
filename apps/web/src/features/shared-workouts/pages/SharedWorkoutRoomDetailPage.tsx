...
import { sharedWorkoutRoomDetailQueryOptions } from '../api/shared-workout-query-options';
import {
...
export function SharedWorkoutRoomDetailPage() {
  ...
  return (
    <main className="mx-auto flex w-full max-w-3xl flex-1 flex-col gap-6">
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-[var(--muted)]">
              Statut : <span className="font-medium text-[var(--foreground)]">{statusLabel}</span>
            </p>
          </div>
          {room.isOwner &&
      ...
      {offline ? (
        <p role="status" className="rounded-[var(--radius)] border border-[var(--border)] bg-[var(--card)] p-3 text-sm">
      ...
      >
        <h2 id="members-heading" className="flex items-center gap-2 text-lg font-semibold">
        ...
        <p className="mt-3 text-xs text-[var(--muted)]">
          Les invitations seront disponibles prochainement.
        </p>
      </section>
      ...
      <section className="flex flex-col gap-2 text-sm text-[var(--muted)]">
        ...
        {room.startedAt ? (
          <p>
            Démarrée :{' '}
            {new Date(room.startedAt).toLocaleString('fr-FR')}
          </p>
        ) : null}
        {room.completedAt ? (
          <p>
            Terminée :{' '}
            {new Date(room.completedAt).toLocaleString('fr-FR')}
          </p>
        ) : null}
        {room.cancelledAt ? (
          <p>
            Annulée :{' '}
            {new Date(room.cancelledAt).toLocaleString('fr-FR')}
          </p>
        ) : null}
      </section>
      ...
      {room.status === 'LOBBY' && canMutateLifecycle ? (
        <div className="flex flex-col gap-3">
          ...
          <Button
            ...
            disabled={pending}
            onClick={() =>
              void runLifecycle('cancel', 'Annuler cette salle ?')
            }
          >
            Annuler la salle
          </Button>
      ...
      {!room.isOwner && room.status !== 'COMPLETED' && room.status !== 'CANCELLED' ? (
        <p className="text-sm text-[var(--muted)]">
